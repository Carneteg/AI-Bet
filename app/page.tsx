  ...
  return (
    <div>
      ...
      <section className="text-center py-16 px-4">
        <div className="inline-flex items-center gap-2 bg-brand/10 border border-brand/30 rounded-full px-4 py-1 text-brand text-sm font-medium mb-6">
          {source === "live" ? (
            <><span className="w-2 h-2 rounded-full bg-accent-green animate-pulse inline-block" /> Live · {today}</>
          ) : (
            <><span className="w-2 h-2 rounded-full bg-accent-yellow inline-block" /> Exempel-data · Lägg till API-nyckel för live</>
          )}
        </div>
      ...
      <section className="mb-12 bg-surface-card border border-brand/20 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className={`text-2xl font-extrabold ${strategy.confidenceScore >= 70 ? "text-accent-green" : "text-accent-yellow"}`}>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">Högst value score idag</h2>
          <Link href="/matches" className="text-brand hover:underline text-sm font-medium">
            Visa alla →
          </Link>
        </div>
      ...
      <section className="mt-16 border-t border-slate-700/50 pt-12 grid md:grid-cols-3 gap-6">
        <div className="bg-surface-card border border-slate-700 rounded-2xl p-5">
          ...
          <p className="text-slate-400 text-sm leading-relaxed">
            ...
            från attack/defense-ratings och xG. Mer träffsäker än råa vinstfrekvenser.
          </p>
        </div>
